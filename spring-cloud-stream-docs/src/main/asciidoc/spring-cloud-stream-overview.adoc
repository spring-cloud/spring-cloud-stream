...
[NOTE]
====
Reactive programming support requires the use of Reactor 3.0.0 and higher. `spring-cloud-stream-reactive` will transitively retrieve the proper version, but it is possible for the project structure to manage the version of the `io.projectreactor:reactor-core` to an earlier release. This is the case for projects generated via Spring Initializr, which will override the Reactor version to `2.0.8.RELEASE`. In such cases you must ensure that the proper version of the artifact is released. This can be simply achived by adding a direct dependency on `io.projectreactor:reactor-core` with a version of `3.0.0.RC1` or later.
====
