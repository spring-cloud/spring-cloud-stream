...
Spring Cloud Stream builds upon Spring Boot and uses Spring Integration to provide connectivity to message brokers.
...
The `@EnableBinding` takes one or more interfaces as parameters (in this case, the parameter is a single `Sink` interface).
...
Complementary to its Spring Integration support, Spring Cloud Stream provides its own `@StreamListener` annotation, modeled after other Spring Messaging annotations (e.g.
`@MessageMapping`, `@JmsListener`, `@RabbitListener`, etc.).
