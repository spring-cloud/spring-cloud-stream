...
When using Kafka binder for the outbound channel, the `asynchronous` Kafka producer is used by default.
To use synchronous Kafka producer, the outbound channel requires to have the binding property
`spring.cloud.stream.bindings.<channelName>.syncProducer=true`
