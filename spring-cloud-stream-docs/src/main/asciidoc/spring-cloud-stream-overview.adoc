...
Spring Cloud Stream is a framework for building message-driven microservices, and more specifically stream processing applications.
...
Spring Cloud stream provides an opinionated configuration of message brokers, introducing the concepts of persisten pub/sub semantics, consumer groups and partitions across several middleware vendors.
...
The interfaces `Source`, `Sink` and `Processor` are provided off the shelf, but you can define others.
...
It is optionally parameterized by a channel name - if the name is not provided the method name is used instead.
...
Different `Binder` implementations exist for different types of middleware, such as Kafka, Rabbit MQ, Redis or Gemfire, and an extensible API allows you to write your own `Binder`.
...
Taking the sink example from the previous section, providing the `spring.cloud.stream.bindings.input.destination=raw-sensor-data` property to the application will cause it to read from the `raw-sensor-data` Kafka topic, or from a queue bound to the `raw-sensor-data` exchange in Rabbit MQ.
...
For message-driven microservices and stream processing scenarios, communication between different applications typically follows a publish-subscribe pattern, with data being broadcast through shared topics.
...
To go with it, two other concepts are central to Spring Cloud Stream: consumer groups and durability.
...
Spring Cloud Stream allows you to write applications by either using the Spring Integration directly, and it provides a `@StreamListener` annotation of its own modeled by the other similar Spring Messaging annotations (e.g. `@MessageMapping`, `@JmsListener`, `@RabbitListener`, etc.).
...
On middlewares that do not support headers per se, Spring Cloud Stream provides its own mechanism of wrapping outbound messages in an envelope of its own, automatically.
On middlewares that support headers, Spring Cloud Stream applications may receive messages with a given content type from non-Spring Cloud Stream applications.
