...
republishToDlq::
By default, messages that fail after retries are exhausted and rejected.
If a dead-letter queue (DLQ) is configured the binder republishes failed messages to the dead-letter queue.
When this property is set to `true` (the default), the binder will add additional headers to the failed message, 
including the exception message and stack trace from the cause of the final failure.
When `false`, messages will be routed to the DLQ in its original form without extra headers.
