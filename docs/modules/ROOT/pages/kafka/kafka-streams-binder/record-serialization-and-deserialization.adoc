...
NOTE: If you provide `Serde` as abover per input binding, then that will take higher precedence and the binder will stay away from any `Serde` inference.
...
If that doesn't work, then it falls back to `JacksonJsonSerde` provided by the Spring Kafka project, but first look at the default `Serde` configuration to see if there is a match.
...
If you don't want the native encoding provided by Kafka, but want to use the framework provided message conversion, then you need to explicitly disable native encoding since native encoding is the default.
