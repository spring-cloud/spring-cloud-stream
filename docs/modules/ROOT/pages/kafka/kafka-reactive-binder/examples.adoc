...
Also, starting with version 4.0.3, you can set the consumer property `reactiveAutoCommit` to `true` and the binder will automatically commit the offsets after the records returned by each poll are processed.
