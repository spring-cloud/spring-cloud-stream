...
See also parameter 'spring.kafka.producer.properties.metadata.max.age.ms' to configure update intervall.
Due to some limitations it is not possible to use a 'partition-key-expression' which reference the 'payload' of the message, the mechanism will be disabled in that case.
The general behaviour is disabled by default and can be enabled using configuration parameter 'producer.dynamicPartitionUpdatesEnabled=true'.
