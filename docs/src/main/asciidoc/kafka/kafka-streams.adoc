...
When Micrometer is on the classpath of a Spring Cloud Stream Kafka Streams binder based application, its consumer topic offset lags are collected as a gauge metric -
`spring.cloud.stream.binder.kafka.offset`.

The offset lag is computed for each consumed topic and partition whenever the mentioned metric is queried.
When this computation fails (i.e. because of a timeout) a fallback value is used that is periodically calculated in the background.
...
refer to the <<kafka-streams-binder-properties, properties section>> for more information.
...
metrics.offsetLagMetricsInterval::
