...
[[pollable-consumer-poller]]
==== Configuring a Poller for a PollableMessageSource

As the above examples show, the PollableConsumer's `receive` method returns immediately whether there is a message or not.
It requires a enclosing loop to invoke it periodically, typically running in a separate thread.
The `PollableMessageSourcePoller` serves this purpose.
The following example shows how to configure a PollableMessageSourcePoller which will run a scheduled task, configured via PollerMetadata, to invoke a MessageHandler for every message received.

[source,java]
----
@SpringBootApplication
@EnableBinding(PollerApplication.PolledSource.class)
