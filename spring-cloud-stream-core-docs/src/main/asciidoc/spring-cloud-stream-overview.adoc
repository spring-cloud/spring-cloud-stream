...
Retrieves all versions of a schema given a subject and a format.

Response is an array of schema objects in JSON format, with the following fields:
...
[NOTE]
====
The default client caches all responses locally to avoid unnecessary network calls. It leverages Spring caching abstraction, so you can provide
an external cache provider such as eh-cache or hazelcast if you want to have distributed cache control and invalidation. Schemas are append only entities
but deletes may occur, make sure you configure your external cache provider to invalidate the cache from time to time to avoid synchronization issues.
====
