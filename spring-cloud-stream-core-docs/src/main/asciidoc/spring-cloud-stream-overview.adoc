...
As a rule of thumb, for outbound messages, if your payload is of `byte[]` type, the framework will skip the entire conversion logic, and just write those bytes to the wire, setting the `contentType` of the message (if absent) to the default configured for the channel.
