...
[NOTE]
====
Each child application in the aggregate application needs to be packaged into a separate package. This is needed to avoid classpath scanning performed by the annotation `SpringBootApplication` on the configuration classes inside the same package.
...
In the above example, Source, Processor and Sink application classes need to be in separate packages.
====
