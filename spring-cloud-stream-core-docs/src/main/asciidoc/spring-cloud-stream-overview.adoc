...
[NOTE]
====
Each child application in the aggregate application needs to be packaged into a separate package.
This is needed to avoid classpath scanning performed by the annotation `SpringBootApplication` on the configuration classes inside the same package.
When the component scan kicks in by the `SpringBootApplication` and if the other child applications are also in the same package, then the binding configurations get mixed up and the application behaves erroneously.
In the above example, it can be seen that the Source, Processor and Sink application classes are grouped in separate packages.
====
