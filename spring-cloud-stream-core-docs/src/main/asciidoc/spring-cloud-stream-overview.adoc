...
You can disable error channel processing by setting `spring.cloud.stream.bindings.<channel>.consumer.polled.recoverable` to `false`.
In that case, exceptions thrown by the listener will have the imediate affect of requeuing or rejecting the message.
Again, throw a `RequeueCurrentMessageException` to requeue, as discussed above.

IMPORTANT: When the error channel is disabled, any dead-letter mechanisms implemented by binders will also be disabled since those mechanisms consume from the error channel.
...
Default: `-1`.
polled.recoverable::
When set to true (default) exceptions thrown by the polled consumer callback will result in an ErrorMessage sent to the consumer's error channel (and the global Spring Integration errorChannel).
Set to `false` if, say, you want to requeue messages by throwing `RequeueCurrentMessageException` s.
See <<polled-errors>>.
+
Default: `true`.
useNativeDecoding::
