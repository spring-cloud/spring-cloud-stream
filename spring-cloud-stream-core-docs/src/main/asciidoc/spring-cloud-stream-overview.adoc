...
Using the specific 'partitionSelectorExpression':

----
spring.cloud.stream.bindings.output.producer.partitionKeyExpression=payload.id
spring.cloud.stream.bindings.output.producer.partitionSelectorExpression=payload.group
----

or using the 'partitionSelectorClass':

----
spring.cloud.stream.bindings.output.producer.partitionKeyExpression=payload.id
spring.cloud.stream.bindings.output.producer.partitionSelectorClass=com.example.MySelector
----

Additional properties can be configured for more advanced scenarios, as described in the following section.

===== Accessing application context for PartitionKeyExtractorClass

In cases where 'PartitionKeyExtractorClass' needs to access the underlying 'Spring application context' (to resolve application properties etc.,) the following can be done:

You can configure your 'PartitionKeyExtractorClass' implementation as a @Bean in the application context with a bean name of the fully qualified class name, that bean will be fully initialized by Spring and used by the binder.
If the binder doesn't find a bean with that name, it will create one (and add it to the context), but it won't get a full Spring initialization.
...
Alternatively, you can implement 'ApplicationContextAware' and the application context will be injected and you can get values from its environment (as well as beans etc).
