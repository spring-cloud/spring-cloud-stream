...
Please note, the normalization exists as an attempt to make downstream consumers of those metrics capable of reading the same data regardless of how the user set it (`--spring.application.name` or `SPRING_APPLICATION_NAME` would always yield `spring.application.name`)
