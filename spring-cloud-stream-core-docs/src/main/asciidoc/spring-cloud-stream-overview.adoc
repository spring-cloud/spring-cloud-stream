...
In order to do so, you can create a class of type `org.springframework.messaging.converter.MessageConverter` and make it available in the application context.
...
Here is an example of creating message converter bean inside the application:
...
Here is an example of configuring it in a sink application registering the Apache Avro, without a predefined schema:
