...
The framework now depends on a `contentType` to be present as a header in order to know how serialize/deserialize a payload.
This have simplified quite a lot the decisions the framework used to make on behalf of the user, removing some bugs that would show on edge cases.
...
As a rule of thumb, for outbound messages, if your payload is of `byte[]` type, the framework will skip the entire conversion logic, and just write those bytes to the wire.

If you send a `byte[]` message but do not provide the `contentType` header, the framework will use the default that is set for the channel.
