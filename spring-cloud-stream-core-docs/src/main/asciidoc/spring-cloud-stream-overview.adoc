...
Source Payload represents the 'payload' before conversion and Target Payload represents the 'payload' after conversion.
The message type-conversion can occur either at the 'producer' side (outbound) or at the 'consumer' side (inbound).

|===
|Source Payload |Target Payload |content-type header at source payload | content-type at target payload after conversion | Comments
...
This is because the payload at the module's outbound channel is already a String so no conversion will be applied at runtime.
It is also important to note that when the POJO deserialization occurs, the class to be de-serialized needs to be on the classpath at the application where the deserialization takes place.
====
