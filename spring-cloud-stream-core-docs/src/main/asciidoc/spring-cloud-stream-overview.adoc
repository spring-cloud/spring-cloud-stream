...
[NOTE]
====
A decorator bean of type `CachingRegistryClient` is available if you want to cache responses from `fetch` operations. In your application
just create an instance of that bean and it will replace the default version. It uses Spring caching abstraction, so any cache provider that
you have on your application can be used as a cache manager for the client.
====
