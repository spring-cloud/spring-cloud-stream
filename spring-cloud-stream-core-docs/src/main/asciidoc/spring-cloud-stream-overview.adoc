...
This section goes through the details of various components involved in schema evolution process.
...
For Spring Boot applications that have a `SchemaRegistryClient` bean registered with the application context, Spring Cloud Stream auto configures an Apache Avro message converter that uses the schema registry client for schema management.
...
If the user provides a custom converter (as below), then the default `AvroSchemaMessageConverter` bean will not be created.
...
In these examples, note that the mime type value is `avro/bytes`, not the default `application/avro`.
