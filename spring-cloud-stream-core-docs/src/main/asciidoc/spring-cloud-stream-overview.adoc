...
== Metrics Emitter
Spring Cloud Stream provides a module called `spring-cloud-stream-metrics` that can be used to emit any available metric from boot's `/metrics` endpoint to a named channel.
This module allow operators to collect metrics from stream applications without relying on polling their endpoints, which could be on a private network or behind an LB that prevents per instance access.
