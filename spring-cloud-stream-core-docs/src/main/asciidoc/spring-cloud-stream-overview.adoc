...
In cases where 'PartitionKeyExtractorClass' needs to access the underlying application context (to resolve application properties etc.,) the following can be done:

You can configure your strategy implementation as a @Bean in the application context with a bean name of the fully qualified class name, that bean will be fully initialized by Spring and used by the binder.
