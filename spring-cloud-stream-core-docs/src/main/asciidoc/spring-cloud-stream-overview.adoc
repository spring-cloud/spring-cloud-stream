...
When passing the binding service properties for non-self contained aggregate applications, it is required to pass the binding service properties to the aggregate application instead of setting them as 'args' to individual child application.
For instance,
