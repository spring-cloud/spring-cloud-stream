...
For instance, the below example application registers a REST 'controller' with a request mapping on the path '/'. It also has 'router' that resolves the dynamic destinations using the SpEL expression evaluated against the message payload.
